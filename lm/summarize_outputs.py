...
# Note: This is the default path if no argument is provided
DEFAULT_BASE_DIR = Path('/scratch/s4568966/LFD_Final_2025/lm/output/bert/')
# --- End Configuration ---
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Parse and summarize grid search results.'
    )
    parser.add_argument(
        '--path',
        type=str,
        default='',
        help='Optional subdirectory to append to the base path for searching results.',
    )
    args = parser.parse_args()

    # 2. Determine the Final BASE_DIR
    # Use the / operator from pathlib.Path to safely join the paths
    BASE_DIR = DEFAULT_BASE_DIR / args.path
    ...
    # We use glob('**/results.txt') to search recursively within BASE_DIR
    # Using '*/results.txt' only searches immediate subdirectories
    # If the subdirectories are one level deeper, you might need '**/results.txt'
    # Keeping '*/results.txt' as in your original for consistency:
    result_files = list(BASE_DIR.glob('*/results.txt'))

    if not result_files:
        print(f"Error: No 'results.txt' files found in immediate subdirectories of {BASE_DIR}")
        return
